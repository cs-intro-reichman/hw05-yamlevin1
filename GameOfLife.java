...
public class GameOfLife {
	...
	public static int[][] evolve(int[][] board) {
		int[][] evolve = new int[board.length][board[0].length];
		for (int i = 1; i < evolve.length - 1; i++) {
			for (int j = 1; j < evolve[0].length - 1; j++) {
				evolve[i][j] = cellValue(board, i, j);
			}
		}
		return evolve;
	}
	...
	public static int count(int[][] board, int i, int j) {
		int count = 0;
		// checks the row beforw the given cell
		if (board[i - 1][j - 1] == 1) {
			count = count + 1;
		}
		if (board[i - 1][j] == 1) {
		...
		// checks the row beforw of given cell withput him
		if (board[i][j - 1] == 1) {
			count = count + 1;
		}
		if (board[i][j + 1] == 1) {
		...
		// checks the row after the given cell
		if (board[i + 1][j - 1] == 1) {
			count = count + 1;
		}
		if (board[i + 1][j] == 1) {
